                       ...
                       latest = Sys.Date())
{
    ...
    mutate(
        i = variable %in% c("Observed", "No intervention", "Social distancing",
                            "Lockdown with moderate release"),
        j = variable == "Lockdown upper credible interval"
    ) %>%
    ...
    colors <- c("#979799", "#ED553B", "#f2c82e", "#173F5F", "#ED553B", "#f2c82e",
                "#173F5F")

    p <- plot_ly(filter(data, i), x = ~ Dates, y = ~ value, text = ~text,
    ...
    add_trace(data = filter(data, j),
              x = ~Dates, y = ~value, type = "scatter", mode = "line",
