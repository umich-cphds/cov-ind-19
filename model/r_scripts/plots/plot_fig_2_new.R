plot_fig_2 <- function(start.date = as.Date("2020-03-01"))
{
    ...
    print(data)
    data <- add_row(data, !!!doubles) %>% spread(Country, Cases)
    ...
    add_trace(data = data, x = ~Day, y = ~ `3 Days`, showlegend = F,
              line = list(dash = "dash", width = 2, color = colors["3 Days"])
    ) %>%
    add_trace(data = data, x = ~Day, y = ~ `1 Week`, showlegend = F,
              line = list(dash = "dash", width = 2, color = colors["1 Week"])
    ) %>%
    add_trace(data = data, x = ~Day, y = ~ `2 Weeks`, showlegend = F,
              line = list(dash = "dash", width = 2, color = colors["2 Weeks"])
    ) %>%
    add_trace(data = data, x = ~Day, y = ~ India, name = "India", text = ~text,
              line = list(width = 3, color = colors["India"])
    ) %>%
    add_trace(data = data, x = ~Day, y = ~ China, name = "China", text = ~text,
              line = list(width = 3, color = colors["China"])
    ) %>%
    add_trace(data = data, x = ~Day, y = ~ `South Korea`, name = "South Korea", text = ~text,
              line = list(width = 3, color = colors["South Korea"])
    ) %>%
    add_trace(data = data, x = ~Day, y = ~ `Italy`, name = "Italy", text = ~text,
              line = list(width = 3, color = colors["Italy"])
    ) %>%
    add_trace(data = data, x = ~Day, y = ~ `Iran`, name = "Iran",text = ~text,
              line = list(width = 3, color = colors["Iran"])
    ) %>%
    add_trace(data = data, x = ~Day, y = ~ `France`, name = "France", text = ~text,
              line = list(width = 3, color = colors["France"])
    ) %>%
    add_trace(data = data, x = ~Day, y = ~ `Germany`, name = "Germany", text = ~text,
              line = list(width = 3, color = colors["Germany"])
    ) %>%
    add_trace(data = data, x = ~Day, y = ~ `US`, name = "US", text = ~text,
