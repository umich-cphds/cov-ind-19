...
data_caseplot =
  data_caseplot %>%
  mutate(full2 = full)
India_p7a<-ggplot(data = data_caseplot, aes(x = Date, y = dailyCases, group = full)) + 
...
# culumative cases
India_p7b<-ggplot(data = data_caseplot, aes(x = Date, y = Cases, group = full)) +
  ...
  geom_text(data = distinct(data_caseplot %>% select(full, totalcases, State, Datemax)),
            aes(x = Datemax - 18, y = 1750, group = full, label = paste(totalcases, ' total cases', sep = ' '))) +
  labs(title = '',
       subtitle = subtitle,
       caption = caption)
...
data_deathplot =
  data_deathplot %>%
  mutate(full2 = full)
India_p7c <- ggplot(data = data_deathplot, aes(x = Date, y = dailyDeaths, group = full)) +
...
# culumative
India_p7d <- ggplot(data = data_deathplot, aes(x = Date, y = Deaths, group = full)) +
  ...
  geom_text(data = distinct(data_deathplot %>% select(full, totalDeaths, State, Datemax)),
            aes(x = Datemax - 22, y = 140, group = full, label = paste(totalDeaths, ' total deaths', sep = ' '))) +
  labs(title = '',
       subtitle = subtitle,
       caption = caption)
