...
##----------------------------------------------------------------------------------------------------------------
## This code makes barplots of COVID-19 Confirmed Cases/Deaths using current data and forecast data
##----------------------------------------------------------------------------------------------------------------


################## Bar plot with upper confidence limit for India
require(readr)
require(tidyr)
require(ggplot2)
library(dplyr)

predplot <- function(start.date, full.rdata="bar_plot_adj.RData", forecast.ci.rdata="Forecast_CI.RData", data.folder=NULL, plot.folder, plotfilename="Figure3India.png")
{
	...
	datafwide$India.up[which(as.Date(datafwide$Dates, format="%m-%d-%y") >= as.Date(dates2[1], format="%m-%d-%y"))] <- India_confirm_up
  ...
  p <- p + geom_bar(stat = "identity", aes(fill = Country), position = "dodge") +  
    ...
    scale_y_continuous(breaks=mybreaks, limits=c(0,ymax), labels=round(exp(mybreaks),0),sec.axis = sec_axis(~., name = "",breaks=mybreaks,labels=round(exp(mybreaks),0))) +
   	...
   	geom_line(data = datafwide, aes(x=Dates, y=log(India.up), group=1), stat="identity", linetype=2, alpha=1,size=2, color="#F8766D") +
   	geom_segment(aes(x=dates[5], xend=dates[10], y=12.5, yend=12.5),color="#F8766D",size=1.5,linetype=2) +
    annotate(geom="text", label=paste0("Upper credible limit of predicted cases for India"), x=dates[25], y=12.5,color="black",size=5.5) +
  	geom_vline(aes(xintercept=which(levels(Dates) %in% dates2[1])), linetype=2,alpha=0.6,size=1) +

    ggsave(paste0(plot.folder,plotfilename),width =20, height =8)
}


predplot(start.date="02-01-2020", full.rdata="bar_plot_adj_Mar21.RData", forecast.ci.rdata="Forecast_CI_Mar21.RData", data.folder="/Users/dray/OneDrive - Johns Hopkins University/HOPKINS/PlayWithData/COVID-19/eSIR-forecast-plots/", plot.folder="/Users/dray/OneDrive - Johns Hopkins University/HOPKINS/PlayWithData/COVID-19/manuscript/", plotfilename="Figure3India.png")